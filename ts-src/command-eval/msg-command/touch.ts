...
export default class TouchMsgCommand extends EriMsgCommand {
    ...
    constructor() {
        ...
        this._execution = (message) => {
            ...
            if (!tx) {
                TouchMsgCommand.TOUCH_TIMES.set(message.author.id, { interval: undefined, times: 0 })
            }

            tx = TouchMsgCommand.TOUCH_TIMES.get(message.author.id)

            if (!tx) return
            message.react(TouchMsgCommand.REPLY_ARR[Math.min(TouchMsgCommand.REPLY_ARR.length, tx.times)][0])
            message.reply(TouchMsgCommand.REPLY_ARR[Math.min(TouchMsgCommand.REPLY_ARR.length, tx.times)][1])
            ++tx.times

            if (tx.interval) clearTimeout(tx.interval)
            tx.interval = setTimeout(() => TouchMsgCommand.TOUCH_TIMES.delete(message.author.id), 120000)
        }
    }
}