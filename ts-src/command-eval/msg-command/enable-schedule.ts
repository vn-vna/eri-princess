  ...
  constructor() {
    ...
    this._execution = (message) => {
      ...
      if (message.mentions.channels.size != 0) {
        channel = message.mentions.channels.find(
      ...
      if (channel) {
        if (message.guild && message.channel)
          ScheduleMessage.getInstance()?.putSchedule(
            message.guild?.id,
            channel?.id
          )
        message.reply(
          `Tính năng tin nhắn chu kỳ đã được bật trên kênh ${channel.toString()} 👌`
        )
      } else {
        const eMsg = new MessageEmbed()
          .setAuthor({
            ...
            url: process.env.ERI_TOPGG_URL,
          })
          .setColor("#dd0000")
          .setTitle("Đăng kí tin nhắn theo chu kì")
          .setThumbnail(
            process.env.ERI_AVATAR_URL ? process.env.ERI_AVATAR_URL : ""
          )
