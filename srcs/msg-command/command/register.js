...
const signServerIn = (message) => {
    EriMongo.query(async (database) => {
        try {
            ...
            if (!finder) {
                const insVal = await database.collection(EriDbConst.ERI_COLL_SERVERS)
                    .insertOne({ ...defaultServerSettings, server_id: message.guild.id })
                const serverRegistered = await database.collection(EriDbConst.ERI_COLL_SERVERS)
                    .countDocuments()
                if (insVal) {
                    const eMsg = new MessageEmbed()
                        ...
                        .addField(`Server ${message.guild.name} has joined with Eri and ${serverRegistered - 1} other servers!!`,
                ...
                const eMsg = new MessageEmbed()
                    ...
                    .addField(`Server ${message.guild.name} has been registered berfore.`,
            ...
            const eMsg = new MessageEmbed()
                ...
                .addField(`Command register from ${message.guild.name} has been ERROR.`,
                    `I'm sorry for that 😫`)
            message.reply({ embeds: [eMsg] })
        }
    })
