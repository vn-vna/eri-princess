...
class ScheduleGT {
    ...
    static sendGoodNight(client, server_detail) {
        if (server_detail.schedule_gt) {
            ...
            if (crrTime.getUTCHours() > GOODNIGHT_HOUR - server_detail.gmt || (crrTime.getUTCHours() == GOODNIGHT_HOUR - server_detail.gmt && crrTime.getUTCMinutes() >= GOODNIGHT_MINUTE)) {
            ...
            console.log(`Scheduled Event Good night for server id: ${server_detail.server_id} in ${timeLeft} ms`)
            ScheduleGT.GOODNIGHT_INTERVALS.push(setTimeout(() => {
